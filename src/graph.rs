...
pub fn create_graph(steps: &[Step]) -> Result<GraphMap<usize, Require, petgraph::Directed>, Error> {
    ...
    for i in 0..steps.len() {
        ...
        for dep in steps[i].require.iter() {
            ...
            let dep_index = steps.iter().position(|ref step| &step.name == dep).ok_or_else(|| anyhow!("Could not build step graph: `{}` can not be found. defined from `require` on `{}`", dep, steps[i].name))?;
            graph.add_edge(dep_index, i, Require);
        }
        ...
        for dep in steps[i].required_by.iter() {
            ...
            let dep_index = steps.iter().position(|ref step| &step.name == dep).ok_or_else(|| anyhow!("Could not build step graph: `{}` can not be found. defined from `required_by` on `{}`", dep, steps[i].name))?;
